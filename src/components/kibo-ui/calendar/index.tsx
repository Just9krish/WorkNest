...
import { getDay, getDaysInMonth, isSameDay, parseISO } from "date-fns";
...
export type CalendarBodyProps = {
  ...
  children: (props: { feature?: Feature; event?: CalendarEvent }) => ReactNode;
...
export const CalendarBody = ({
  ...
  // Memoize items filtering by day to avoid recalculating on every render
  const itemsByDay = useMemo(() => {
    const result: {
      [day: number]: { features?: Feature[]; events?: CalendarEvent[] };
    } = {};
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      if (features) {
        result[day].features = features.filter(feature => {
          return isSameDay(new Date(feature.endAt), dayDate);
        });
      }

      if (events) {
        result[day].events = events.filter(event => {
          try {
    ...
    return result;
  }, [features, events, daysInMonth, year, month]);
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    const featuresForDay = dayItems.features || [];
    const eventsForDay = dayItems.events || [];
    const totalItems = featuresForDay.length + eventsForDay.length;
    const dayDate = new Date(year, month, day);
    ...
    days.push(
      <div
        className={cn(
          ...
          isSameDay(dayDate, new Date()) && "bg-primary/10"
        )}
        key={day}
        onClick={() => onDayClick?.(day, dayDate)}
      >
        <span
          className={cn(
            "font-medium",
            isSameDay(dayDate, new Date()) && "text-primary"
          )}
        >
          {day}
        </span>
        <div className="space-y-1">
          ...
          {eventsForDay
            .slice(0, 3 - featuresForDay.length)
            .map(event => children({ event }))}
        </div>
        {totalItems > 3 && (
...
export const CalendarItem = memo(
  ({ feature, event, className, onClick }: CalendarItemProps) => {
    if (feature) {
      return (
        <div
